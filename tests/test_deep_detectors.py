...
@pytest.mark.parametrize(
    ...
    [
        (
            deep.VariationalAutoencoderDetector,
            {"epochs": 5, "latent_dim": 4, "hidden_dim": 8, "patience": 2},
        ),
        (deep.LSTMAutoencoderDetector, {"epochs": 3, "hidden_size": 4, "patience": 2}),
        (
            deep.TransformerDetector,
            {"epochs": 3, "d_model": 8, "nhead": 2, "patience": 2},
        ),
    ],
)
...
def test_variational_autoencoder_improves_reconstruction(toy_data: np.ndarray) -> None:
    ...
    detector.fit(
        toy_data, epochs=5, latent_dim=3, hidden_dim=6, patience=2, validation_split=0.2
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            deep.MADGANDetector,
            {"epochs": 5, "latent_dim": 4, "batch_size": 8, "patience": 2},
        ),
    ],
)
def test_gan_based_detectors_return_scores(detector_cls, fit_kwargs, toy_data) -> None:
    detector = detector_cls()
    detector.fit(toy_data, validation_split=0.2, **fit_kwargs)
